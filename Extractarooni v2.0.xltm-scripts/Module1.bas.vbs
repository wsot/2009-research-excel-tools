...
Sub buildTuningCurves()
    ...
    outputDir = Worksheets("Settings").Range("B10").Value
    
    If doImport Then
        While Worksheets("Variables (do not edit)").Range("H" & i).Value <> ""
            Call Worksheets("Output").Range("A1:ZZ1000").ClearContents
            Call Worksheets("StdDev").Range("A1:ZZ1000").ClearContents
            Call Worksheets("Means").Range("A1:ZZ1000").ClearContents
            Call Worksheets("N").Range("A1:ZZ1000").ClearContents
            theBlock = Worksheets("Variables (do not edit)").Range("H" & i).Value
            Call processImport(False)
            Application.DisplayAlerts = False
            Call Workbooks(1).SaveAs(outputDir & "\" & theBlock, 52)
            i = i + 1
        Wend
    End If
End Sub
...
Sub processImport(importIntoSigmaplot As Boolean)
    ...
    ReDim arrHistTmp(Worksheets("Settings").Range("B3").Value - 1)
    'ReDim arrHistTmp(31)
...
Sub writeAxes(colLabels As Variant, rowLabels As Variant, iColOffset, iRowOffset, zOffset)
    ...
    For j = 0 To UBound(rowLabels)
        ...
        Worksheets("StdDev").Cells(iRowOffset + j + 2 + zOffset, iColOffset + 1).Value = rowLabels(j)
        Worksheets("Means").Cells(iRowOffset + j + 2 + zOffset, iColOffset + 1).Value = rowLabels(j)
        Worksheets("N").Cells(iRowOffset + j + 2 + zOffset, iColOffset + 1).Value = rowLabels(j)
    Next
    For j = 0 To UBound(colLabels)
        Worksheets("Output").Cells(iRowOffset + zOffset + 1, j + 2).Value = colLabels(j)
        Worksheets("StdDev").Cells(iRowOffset + zOffset + 1, j + 2).Value = colLabels(j)
        Worksheets("Means").Cells(iRowOffset + zOffset + 1, j + 2).Value = colLabels(j)
        Worksheets("N").Cells(iRowOffset + zOffset + 1, j + 2).Value = colLabels(j)
    Next
...
Sub transferToSigmaplot(xCount, yCount, zOffsetSize, iColOffset, iRowOffset, lMaxHistHeight)
...
Function buildEpocList(objTTX, AxisEp, bReverseOrder)
    ...
    If AxisEp = "Channel" Then
        For i = 1 To Worksheets("Settings").Range("B3").Value
    ...
    For i = 0 To UBound(arrOtherEpocKeys(iOtherEpocNum))
        ...
        Else
            ...
            If xOffset = 1 And yOffset = 1 Then
                ...
                Worksheets("N").Cells(iRowOffset + (i * zOffset), iColOffset + 1).Value = Left(strAddedTitle, Len(strAddedTitle) - 2)
                Worksheets("Means").Cells(iRowOffset + (i * zOffset), iColOffset + 1).Value = Left(strAddedTitle, Len(strAddedTitle) - 2)
                Worksheets("StdDev").Cells(iRowOffset + (i * zOffset), iColOffset + 1).Value = Left(strAddedTitle, Len(strAddedTitle) - 2)
...
Sub writeResults(ByRef objTTX, xOffset, yOffset, zOffset, iChanNum, ByRef lMaxHistHeight)
    ...
    If IsArray(varReturn) Then
        ...
        If xAxisEp = "Channel" Then
            ...
            Worksheets("Means").Cells(yOffset + iRowOffset + zOffset + 1, xOffset + iColOffset + 1).Value = histMean
            Worksheets("StdDev").Cells(yOffset + iRowOffset + zOffset + 1, xOffset + iColOffset + 1).Value = histStddev
            Worksheets("N").Cells(yOffset + iRowOffset + zOffset + 1, xOffset + iColOffset + 1).Value = nSweps
